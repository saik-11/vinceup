...
/** Format a "HH:mm" time with an optional timezone abbreviation like "2:00 PM (IST)" */
export function formatTimeWithTimezone(timeString, timezone) {
  ...
  let timeLabel = `${h12}:${min} ${ampm}`;
  
  if (timezone) {
    try {
      const d = new Date();
      const tzFormatter = new Intl.DateTimeFormat("en-US", { timeZoneName: "short", timeZone: timezone });
      const tzParts = tzFormatter.formatToParts(d);
      const tzStr = tzParts.find((p) => p.type === "timeZoneName")?.value;
      timeLabel += ` (${tzStr || timezone})`;
    } catch (e) {
      timeLabel += ` (${timezone})`;
    }
  }
