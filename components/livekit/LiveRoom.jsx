...
import {
  AlertCircle, CalendarClock, Circle, Copy, Info, ShieldCheck,
  Users, X, Hand, Mic, MicOff, Video as VideoIcon, VideoOff, UserPlus,
} from "lucide-react";
...
const panelSlideVariants = {
  ...
  visible: { x: 0, opacity: 1, transition: { type: "spring", stiffness: 320, damping: 32, mass: 0.8 } },
  exit: { x: "100%", opacity: 0, transition: { type: "spring", stiffness: 400, damping: 40, mass: 0.6 } },
};
...
/* ─── Joining toast helper ─── */

function useParticipantJoinNotifier(room) {
  useEffect(() => {
    if (!room) return;
    const handle = (participant) => {
      ...
      toast(
        <span className="flex items-center gap-2 text-sm">
          ...
          <span><strong>{name}</strong> joined the call</span>
        </span>,
        { duration: 3000 },
      );
    };
    room.on(RoomEvent.ParticipantConnected, handle);
    return () => room.off(RoomEvent.ParticipantConnected, handle);
  }, [room]);
}
...
export function LiveRoom({
  room, roomName = "Live Session", participants, connectionState, error,
  joinedAt, initialMicEnabled = true, initialCamEnabled = false,
  onLeave, className,
}) {
  ...
  /** Timestamp (ms) when recording actually started — null when not recording */
  const recordingStartRef = useRef(null);
  const [recordingSeconds, setRecordingSeconds] = useState(0);
  /** Show confirmation dialog before starting a recording */
  ...
  useParticipantJoinNotifier(room);

  // ── Total elapsed time since join ──
  useEffect(() => {
  ...
  useEffect(() => {
    if (!room) return;
    const handle = (speakers) => setActiveSpeakers(speakers.map((s) => s.identity));
    handle(room.activeSpeakers ?? []);
    room.on(RoomEvent.ActiveSpeakersChanged, handle);
    return () => room.off(RoomEvent.ActiveSpeakersChanged, handle);
  }, [room]);

  // ── Data channel (hand raise + recording) ──
  useEffect(() => {
    if (!room) return;
      ...
      try {
        ...
        if (parsed.topic === HAND_RAISE_TOPIC) {
          setRaisedHands((prev) => {
            ...
            parsed.raised ? next.add(parsed.identity) : next.delete(parsed.identity);
            return next;
          });
        }
        if (parsed.topic === RECORDING_TOPIC && participant?.identity !== localIdentity) {
          const isNowRecording = parsed.recording;
  ...
  const togglePanel = (panel) => setActivePanel((c) => (c === panel ? null : panel));
  const closePanel = () => setActivePanel(null);

  /** Called when user clicks the recording button in ControlBar */
  const handleToggleRecording = useCallback(() => {
    if (!isRecording) {
      ...
      // Stop immediately
      doSetRecording(false);
    }
  }, [isRecording]); // eslint-disable-line react-hooks/exhaustive-deps

  function doSetRecording(next) {
    setIsRecording(next);
    if (next) {
      recordingStartRef.current = Date.now();
    } else {
    ...
    queueMicrotask(() => toast.info(next ? "Recording started" : "Recording stopped"));
  }

  const panelTitle =
    activePanel === "chat" ? "In-call messages" :
    activePanel === "participants" ? "Participants" :
    activePanel === "details" ? "Meeting details" : "";
        ...
        <AlertDialogContent className="border-white/10 bg-slate-900 text-white">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-slate-400">
              All participants will be notified that this session is being recorded. Recording will
              capture audio and video for all participants.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel
              onClick={() => setShowRecordingConfirm(false)}
              className="border-white/10 bg-white/5 text-white hover:bg-white/10"
            >
              Cancel
            </AlertDialogCancel>
            <AlertDialogAction
              onClick={() => { setShowRecordingConfirm(false); doSetRecording(true); }}
              className="bg-red-600 text-white hover:bg-red-700"
            >
              <Circle className="mr-1.5 h-3 w-3 fill-white" />
              Start recording
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
        ...
        <header className="relative z-10 flex shrink-0 items-center justify-between gap-3 border-b border-white/8 bg-slate-950/75 px-3 py-2.5 backdrop-blur-xl sm:px-5 sm:py-3">
          ...
          <div className="flex min-w-0 items-center gap-3">
            <Image
              ...
              alt="VinceUP"
              loading="eager"
              className="h-auto w-20 shrink-0 brightness-0 invert sm:w-24"
            />
          ...
          <div className="flex shrink-0 items-center gap-1.5 sm:gap-2">
            ...
            >
              ...
              {isRecording ? `REC ${formatDuration(recordingSeconds)}` : "Not recording"}
            </Badge>
        ...
        <div className="relative z-10 flex min-h-0 flex-1 overflow-hidden">
            ...
            <AnimatePresence mode="wait">
              {panelOpen && activePanel === "chat" && (
                <motion.div key="chat-panel" variants={panelSlideVariants} initial="hidden" animate="visible" exit="exit" className="flex w-[360px] shrink-0 flex-col xl:w-[400px]">
                  <ChatPanel room={room} localIdentity={localIdentity} onClose={closePanel} className="h-full w-full border-l border-white/10 bg-slate-950/95 shadow-2xl shadow-black/40 backdrop-blur-xl" />
                </motion.div>
              )}
              {panelOpen && activePanel !== "chat" && (
                <motion.aside key={activePanel} variants={panelSlideVariants} initial="hidden" animate="visible" exit="exit" className="flex w-[360px] shrink-0 flex-col border-l border-white/10 bg-slate-950/95 shadow-2xl shadow-black/40 backdrop-blur-xl xl:w-[400px]">
          ...
          {!isDesktop && !isMobile && (
            <>
              ...
              <AnimatePresence mode="wait">
                {panelOpen && activePanel === "chat" && (
                  <motion.div key="chat-panel-tablet" variants={panelSlideVariants} initial="hidden" animate="visible" exit="exit" className="absolute inset-y-0 right-0 z-30 flex w-full max-w-sm flex-col">
                    <ChatPanel room={room} localIdentity={localIdentity} onClose={closePanel} className="h-full w-full border-l border-white/10 bg-slate-950/95 shadow-2xl shadow-black/40 backdrop-blur-xl" />
                  </motion.div>
                )}
                {panelOpen && activePanel !== "chat" && (
                  <motion.aside key={activePanel} variants={panelSlideVariants} initial="hidden" animate="visible" exit="exit" className="absolute inset-y-0 right-0 z-30 flex w-full max-w-sm flex-col border-l border-white/10 bg-slate-950/95 shadow-2xl shadow-black/40 backdrop-blur-xl">
                    <PanelHeader title={panelTitle} icon={panelIcon} onClose={closePanel} />
                    <div className="min-h-0 flex-1 overflow-y-auto">{renderPanelContent()}</div>
                  </motion.aside>
                )}
  ...
  return (
    <div className={cn("flex items-center gap-3 rounded-xl px-3 py-2.5 transition-colors", isActiveSpeaker && "bg-cyan-500/8", hasRaisedHand && "bg-amber-500/8")}>
      ...
      <div className="flex items-center gap-1.5">
        ...
        <span className={cn("flex h-7 w-7 items-center justify-center rounded-lg", cameraEnabled ? "bg-emerald-400/10 text-emerald-300" : "bg-white/5 text-slate-500")} title={cameraEnabled ? "Camera on" : "Camera off"}>
          {cameraEnabled ? <VideoIcon className="h-3.5 w-3.5" /> : <VideoOff className="h-3.5 w-3.5" />}
        </span>
        <span className={cn("flex h-7 w-7 items-center justify-center rounded-lg", micEnabled ? "bg-emerald-400/10 text-emerald-300" : "bg-red-500/10 text-red-400")} title={micEnabled ? "Mic on" : "Mic muted"}>
...
function MeetingDetailsContent({ room, roomName, elapsedSeconds, participants, isRecording, recordingSeconds }) {
    ...
    <div className="space-y-5 p-4 text-sm text-slate-300">
      <section className="rounded-xl border border-white/10 bg-white/3 p-4">
        ...
        <Button
          ...
          onClick={() => { navigator?.clipboard?.writeText?.(roomName); toast.success("Room name copied!"); }}
        >
        ...
        <div className="mt-3 flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-xs text-slate-500">
              {isRecording
                ? `Recording for ${formatDuration(recordingSeconds)}`
