    ...
    <DropdownMenuTrigger asChild>
      ...
      >
        ...
        <motion.span
          ...
          className="absolute top-2 right-2.5 size-2 rounded-full bg-red-500 ring-2 ring-background"
        />
      </Button>
    </DropdownMenuTrigger>
...
// ─── Main Component ───
const AuthButtons = ({ layout }) => {
  const { token, user, logout } = useAuth();
  const { resolvedTheme, setTheme } = useTheme();

  if (!token) {
    return (
      <motion.div className="flex items-center gap-4" {...FADE_IN}>
        <Button
          asChild
          variant="ghost"
  ...
  return (
    <motion.div className="flex items-center gap-3" {...FADE_IN}>
      <NotificationBell />
