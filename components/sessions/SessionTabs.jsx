...
const emptyMessages = {
  upcoming: "You don't have any upcoming sessions at the moment.",
  requests: "You don't have any pending session requests.",
  today: "You don't have any sessions scheduled for today.",
  past: "You don't have any past sessions yet.",
};

export const SessionTabs = ({ tabs, onAction }) => {
  return (
    <Tabs defaultValue="upcoming" className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-sm flex flex-col overflow-hidden">
      <TabsList className="bg-transparent h-auto p-0 pb-[3.8rem] border-b border-slate-200 dark:border-slate-800 w-full grid grid-cols-4 rounded-none">
        {categories.map((cat, index) => {
          const count = tabs[cat.id]?.length ?? 0;
      ...
      <AnimatePresence>
        {categories.map((cat) => (
          <TabsContent key={cat.id} value={cat.id} className="mt-0 outline-none">
            ...
            >
              {(tabs[cat.id] ?? []).length > 0 ? (
                (tabs[cat.id] ?? []).map((session, idx) => (
                  <SessionCard
                    key={session.id}
                    session={session}
                    category={cat.id}
